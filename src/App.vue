  ...
  <div v-else>
    ...
    <header class="header">
      ...
      <div class="acciones">
        ...
        <button class="btnGrande" @click="menuUser = !menuUser">
          Usuario
        </button>
        ...
        <button class="btnGrande" @click="showM = !showM">
          Menu
        </button>
...
export default {
  ...
  async mounted() {
  // Esperar que los productos ya estén antes de cargar carrito
  await this.$nextTick()
  ...
  methods: {
    ...
    mostrar(msg, color = 'red') {
      ...
      this._mensajeColor = color
      setTimeout(() => { this.mostrarMensaje = false }, 2500)
    },

    // Carga el carrito desde el backend y lo mapea al formato local
    async cargarCarritoBackend() {
  try {
    const data = await obtenerCarrito()  // { cart_id, items, total }
    this.carrito = data.items.map(item => {
      const prod = this.productos.find(p => p.id === item.product_id)
      if (!prod) return null
      return { ...prod, cantidad: item.quantity, itemId: item.id }
    }).filter(Boolean)
  } catch { /* silencioso */ }
},
    ...
    async add(p) {
      ...
      if (existente) {
        existente.cantidad++
        // Si tiene itemId, ya existe en backend → actualizar
        if (existente.itemId) {
          try { await actualizarItem(existente.itemId, existente.cantidad) } catch {}
        }
      } else {
        this.carrito.push({ ...p, cantidad: 1 })
        // Agregar en backend y guardar el itemId que devuelve
        try {
          const res = await agregarItem(p.id, 1)
          const nuevo = this.carrito.find(c => c.id === p.id)
          if (nuevo) nuevo.itemId = res.item?.id
        } catch (err) {
          console.warn('No se pudo sincronizar con backend:', err.message)
        }
      }
    ...
    async incrementarItem(idx) {
      ...
      item.cantidad++
      if (item.itemId) {
        try { await actualizarItem(item.itemId, item.cantidad) } catch {}
      }
    },
    ...
    async eliminarItemCarrito(idx) {
      const item = this.carrito[idx]
      if (item.itemId) {
        try { await eliminarItem(item.itemId) } catch {}
      }
