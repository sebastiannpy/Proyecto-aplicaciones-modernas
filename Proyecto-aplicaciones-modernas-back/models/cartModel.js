...
import { getProductsSelectFields } from "../services/productsSchema.js";
...
export async function getCartWithItems(userId) {
  ...
  const productFields = await getProductsSelectFields();

  const { data: items, error } = await supabaseAdmin
    .from("cart_items")
    .select(`id, quantity, product_id, products (${productFields})`)
    .eq("cart_id", cart.id);
